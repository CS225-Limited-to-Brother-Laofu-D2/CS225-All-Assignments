...
//
//  Modified by Group D2 for directed graph
...
{
    // Added by group D2, since it is a digraph now, we must add second vertex to the graph(if it is not in the vertice list)
    addvertex(second);
    // Now follow KD.
    vertex<T> * pt = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
...
//Return a list containing the outgoing edges, output "nope" if none.
template<class T> edgelist<T> * vertexlist<T>::incomingedges(T item) // Added by group D2
{
    edgelist<T>* result = new edgelist<T>;
    vertex<T> * ptv = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++) // Traversal all the vertices. We need to add the edge with the destination is ITEM.
    {
        if ((*ptv).getedges() != nullptr) // If it has edges, check destination.
        {
            elist<T>* temp = ptv->getedges();
            neighbour<T>* temp_nei = temp->getfirst();
            for(int i = 0 ; i < temp->getlength() ; i++);
            {
                if(temp_nei->getitem() == item)
                {
                    result->add(ptv->getitem(),item);
                }
                temp_nei = temp_nei->getnext();
            }
        }
        ptv = (*ptv).getnext();
    }
...
template<class T> void vertexlist<T>::prettyprint(void)
{
    ...
    for (int i = 0; i < numvertices; i++)
    {
        ...
        cout << "Edges with destination " << (*ptv).getitem() << " :\n";
        pte = incomingedges((*ptv).getitem()); // Added by group D2
        if(pte->getnumedges() == 0)
        {
            cout<<"nope"<<endl;
        }
...
template<class T> vertex<T>* vertexlist<T>::retrieve(int index)
{
...
template<class T> int vertexlist<T>::getindex(T item)
{
    int index = 0;
    if(this->contains(item) == false)
    {
        cout << "Item not in the vertex list" << endl;
        exit(EXIT_FAILURE);
    }
...
template<class T> void graph<T>::addedge(T origin, T destination)
{
    ...
    // (*vertices).addedge(destination, origin); // This is for undirected graph.
    return;
}
...
template<class T> void graph<T>::deleteedge(T origin, T destination)
{
    ...
    // (*vertices).removeedge(destination, origin); // This is for undirected graph.
    return;
}
...
template<class T> edgelist<T> * graph<T>::incomingedges(T item)
{
    return (*vertices).incomingedges(item); // Added by Group D2
}

template<class T> vertex<T>* graph<T>::retrieve(int index)
{
    return vertices->retrieve(index);
}

template<class T> int graph<T>::getindex(T item)
{
    return vertices->getindex(item);
}
...
template<class T> searchforest<T>::searchforest(void)
{
    search_forest = new vector<searchtree<T>*>;
}

template<class T>void searchforest<T>::addtree(searchtree<T>* tree)
{
    search_forest->push_back(tree);
    return;
}

// We will use the print of the forest instead.
template<class T>void searchforest<T>::prettyprint(void)
{
    cout<< "----- This is BFS result for the graph -----"<<endl;
    if(search_forest->size() == 0)
    {
        cout<<"No result from BFS.(The graph might be empty)"<<endl;
        return;
    }
    for(int i = 0; i < search_forest->size(); i++)
    {
        cout<<"Tree "<<i+1<<endl;
        searchtree<T>* tree = new searchtree<T>;
        tree = search_forest->at(i);
        tree->prettyprint();
        cout<<endl;
    }
    cout<< "----- BFS output ended -----"<<endl<<endl;
}
...
template<class T> deeptreenode<T>::deeptreenode(T node,T parent, int pos, int finish, deeptreenode<T> *pt)
{
    nodelabel = node;
    parentlabel = parent;
    dfspos = pos;
...
template<class T>T deeptreenode<T>::parent(void)
{
    return parentlabel;
}
...
template<class T> void deepsearchtree<T>::add(T node,T parent, int pos, int finish)
{
    deeptreenode<T> * newnode = new deeptreenode<T>(node, parent, pos, finish);
    if (first == 0)
    {
        first = newnode;
        last = newnode;
...
// Added
template<class T> deeptreenode<T>* deepsearchtree<T>::find_node(T label)
{
    deeptreenode<T>* result = new deeptreenode<T>;
    result = first;
    while(result->label() != label)
    {
        result = result->next;
    }
    return result;
...
template<class T> int deepsearchtree<T>::ancestor_num(T nodelabel)
{
    int result = 0;
    deeptreenode<T>* temp = new deeptreenode<T>;
    temp = this->find_node(nodelabel);
    T parent = temp->parentlabel;
    while(parent != temp->label())
    {
        result++;
        temp = this->find_node(parent);
        parent = temp->parentlabel;
    }
    return result;
}

template<class T>T* deepsearchtree<T>::ancestorlist(T nodelabel)
{
    int num = this->ancestor_num(nodelabel);
    T* list = new T[num];
    deeptreenode<T>* temp = new deeptreenode<T>;
    temp = this->find_node(nodelabel);
    for(int i = 0; i < num; i++)
    {
        temp = this->find_node(temp->parentlabel);
        list[i] = temp->label();
    }
    return list;
}

template<class T>bool deepsearchtree<T>::is_ancestor(T node, T ancestor)
{
    int num = this->ancestor_num(node);
    T* list = this->ancestorlist(node);
    for(int i = 0; i < num; i++)
    {
        if(ancestor == list[i])
        {
...
template<class T>bool deepsearchtree<T>::contains(T node)
{
    deeptreenode<T>* temp = new deeptreenode<T>;
    temp = first;
    while(temp != last)
...
// Added
template<class T>deepsearchforest<T>::deepsearchforest()
{
    deep_forest = new vector<deepsearchtree<T>*>;
}

template<class T>int deepsearchforest<T>::get_treenum()
{
    return deep_forest->size();
}

template<class T>deepsearchtree<T>* deepsearchforest<T>::retrieve_tree(int index)
{
    if(index >= deep_forest->size())
    {
        cout << "index out of range" << endl;
        exit(EXIT_FAILURE);
    }
    return deep_forest->at(index);
}

template<class T>void deepsearchforest<T>::addtree(deepsearchtree<T>* tree)
{
    deep_forest->push_back(tree);
}

template<class T>void deepsearchforest<T>::prettyprint()
{
    cout << "----- This is DFS result for the graph -----" << endl;
    if(deep_forest->size() == 0)
    {
        cout << "No result from DFS.(The graph might be empty)" << endl;
        return;
    }
    for(int i = 0; i < deep_forest->size(); i++)
    {
        cout<<"Tree "<<i + 1<< endl;
        deepsearchtree<T>* tree = new deepsearchtree<T>;
        tree = deep_forest->at(i);
        tree->prettyprint();
        cout<<endl;
        cout<< "----- DFS output ended -----"<<endl<<endl;
    }
}

template<class T> bool deepsearchforest<T>::is_ancestor(T node, T ancestor)
{
    if(this->get_treenum() == 0)
    {
        cout << "there are no trees!" << endl;
        exit(EXIT_FAILURE);
    }
    //Locate which tree this node is in
    deepsearchtree<T>* tree = new deepsearchtree<T>;
    for(int i = 0; i < this->get_treenum(); i++)
    {
        tree = this->retrieve_tree(i);
        if(tree->contains(node))
            break;
    }
    return tree->is_ancestor(node, ancestor);
}
...
template<class T> searchtree<T> * graphtraversal<T>::bfs(T startnode, markerlist<T>* markedvertices) // Modified
{
        ...
        while ((*currentlayer).isempty() == false)
        {
            ...
            if (neighbours != 0)
            {
                ...
                for (int i = 0; i < (*neighbours).getlength(); i++)
                {
                    ...
                    if ((*output).hasparent(nextnode) == false && markedvertices->ismarked(nextnode) == false)
                    {
...
template<class T> searchforest<T>* graphtraversal<T>::newbfs(T startnode)
{
    searchforest<T>* result = new searchforest<T>;
    searchtree<T>* tree = new searchtree<T>;
    ...
    for(int i = 0; i < this->numvertices(); i++)
    {
        ...
        int index = (this->getindex(startnode) + i) % this->numvertices(); // In case the startnode is not the first one, we need to get the mod.
        vertex<T>*  temp_vertex = this->retrieve(index);
        //If this vertex is not marked
        if(markedvertices->ismarked(temp_vertex->getitem()) == false)
        {
            tree = this->bfs(temp_vertex->getitem(),markedvertices);
            result->addtree(tree);
        }
    }
    return result;
...
template<class T> deepsearchtree<T> * graphtraversal<T>::dfs(T startnode, markerlist<T>* markedvertices, int &dfspos, int &finishingtime)
{
    vertexlist<T> * vertices = graph<T>::vertices;
    if ((*vertices).contains(startnode) == false)
    {
...
template<class T> void graphtraversal<T>::_dfs(T node1, T node2, vertexlist<T> * vertices, deepsearchtree<T> * output, markerlist<T> * markedvertices, int &dfspos, int &finishingtime)
{
    ...
    if (neighbours != 0)
    {
        ...
        for (int i = 0; i < (*neighbours).getlength(); i++)
        {
            ...
            {
                _traverse_treeedge(node1, node2, nextnode, output, dfspos);
                (*markedvertices).mark(nextnode);
...
template<class T> deepsearchforest<T>* graphtraversal<T>::newdfs(T startnode)
{
    deepsearchforest<T>* output = new deepsearchforest<T>;
    deepsearchtree<T>* tree = new deepsearchtree<T>;
    markerlist<T>* markedvertices = new markerlist<T>;
    //For all vertices
    int dfspos = 1;
    ...
    {
        //Start from startnode
        int index = (this->getindex(startnode) + i) % this->numvertices() ;
        vertex<T>*  avertex = this->retrieve(index);
        //If this vertex is not marked
        if(markedvertices->ismarked(avertex->getitem()) == false)
        {
            tree = this->dfs(avertex->getitem(),markedvertices,dfspos, finishingtime);
            //For another tree, dfspos need to increase by 1
            dfspos++;
            output->addtree(tree);
        }
    }
...
template<class T> void graphtraversal<T>::_traverse_treeedge(T parent_node, T first, T second, deepsearchtree<T> * output, int &dfspos)
{
    if ((*output).occurs(first) == false)
        (*output).add(first,parent_node, dfspos);
    dfspos++;
    return;
}
...
template<class T> void graphtraversal<T>::_backtrack(T node1, T node2, deepsearchtree<T> * output, int &dfspos, int &finishingtime)
{
    ...
    {
        (*output).add(node2,node1,dfspos,finishingtime);
    }
    else
...
template<class T> bool graphtraversal<T>::is_acyclic(void)
{
    vertexlist<T>* vertices = graph<T>::vertices;
    //Apply dfs to the first vertex
    deepsearchforest<T>* dfsforest = this->newdfs(vertices->retrieve(0)->getitem());
    //Test every vertices, whether they have backward edges.
    for(int i = 0; i < vertices->getlength(); i++)
    {
        T current_vertex = vertices->retrieve(i)->getitem();
        edgelist<T>* all_edges = this->outgoingedges(current_vertex);
        for(int j = 0;j < all_edges->getnumedges(); j++)
        {
            T destination = all_edges->getedge(j)->destination();
            if(dfsforest->is_ancestor(current_vertex,destination) == true)
            {
                return false;
            }
        }
