...
public:
    ...
    edgelist<T> * incomingedges(T item); // Added
    void prettyprint(void); // only for testing
    vertex<T>* retrieve(int index);  //Added, get the vertex of that index
    int getindex(T item);
private:
    vertex<T> * dummy;
    int numvertices;
...
public:
    ...
    void prettyprint(void); // only for testing
    edge<T>* getedge(int index); // Added
private:
    edge<T> ** reprarray;
    int numedges;
};
...
public:
    ...
    edgelist<T> * incomingedges(T item); // Added
    void prettyprint(void); // only for testing
    vertex<T>* retrieve(int index); // Added
    int getindex(T item); // Added
protected:
...
public:
    deeptreenode(){};   
    deeptreenode(T node,T parent, int pos = 1, int finish = 1, deeptreenode<T> *pt = 0);
    T label(void);  
    T parent(void);
    int dfsnum(void);
    int finishtime(void);
...
public:
    deepsearchtree(void);
    void add(T node,T parent, int pos, int finish = -1);
    void setfinishtime(T node, int finish);
    bool occurs(T node);
    void prettyprint(void); // only for testing
    deeptreenode<T>* find_node(T label);    //Given a label of a node, find it in the tree
    int ancestor_num(T nodelabel); //The number of ancestors of a node. 0 for root
    T* ancestorlist(T nodelabel);    //all ancenstors of the node.
    bool is_ancestor(T node, T ancestor); //Test if "ancestor" is an ancestor of "node"
    bool contains(T node);  //Whether this tree contains this node
private:
...
//Added. Since digraph may not be a fully connected graph, there will be many separate trees, we need to add them all together.
template<class T> class searchforest
{
public:
    searchforest(void); // Constructor
    void prettyprint(void); // Similar to kd.
    void addtree(searchtree<T>* tree);
    vector<searchtree<T>*>* search_forest;
};

//Added
template<class T> class deepsearchforest
{
public:
    deepsearchforest();
    int get_treenum();
    deepsearchtree<T>* retrieve_tree(int index);
    void addtree(deepsearchtree<T>* tree);
    void prettyprint(void);
    bool is_ancestor(T node, T ancestor);
private:
    vector<deepsearchtree<T>*>* deep_forest;
};
...
public:
    ...
    searchtree<T> * bfs (T startnode, markerlist<T>* markedvertices); // Modified
    searchforest<T>* newbfs(T startnode);
    deepsearchtree<T> * dfs (T startnode, markerlist<T>* markedvertices, int &dfspos, int &finishingtime);
    deepsearchforest<T>* newdfs(T startnode);
...
private:
    ...
    void _traverse_treeedge(T parent_node, T first, T second, deepsearchtree<T> * output, int &dfspos);
