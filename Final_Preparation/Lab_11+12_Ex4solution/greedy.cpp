...
template<class T> void AList<T>::discard(T value)
{
    for (int i = 0; i < numitems; i++)
    {
        if ((numitems == maxsize / 4) && (maxsize > minsize))
...
template<class T> vertex<T>::vertex(T item, int col, vertex<T> *pt_next, vertex<T> *pt_prev)
{
    vertexid = item;
    colour = col;
    next = pt_next;
...
template<class T> int vertex<T>::getcolour(void)
{
    return colour;
}
...
template<class T> void vertex<T>::setcolour(int col)
{
    colour = col;
    return;
}
...
template<class T> int vertexlist<T>::numcoloured(void)
{
    int count = 0;
    vertex<T> * pt = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
...
template<class T> void vertexlist<T>::setcolour(T node, int col)
{
    vertex<T> * pt = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
    {
...
template<class T> int vertexlist<T>::getcolour(T node)
{
    vertex<T> * pt = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
    {
...
/* Create a list of the vertices only. */
template<class T> AList<T> *vertexlist<T>::getlist(void)
{
    ...
    for (int i = 0; i < numvertices; i++)
    {
        (*list).append((*ptv).getitem());
        ptv = (*ptv).getnext();
    }
    return list;
}

/* Create a list of the uncoloured vertices only. */
template<class T> AList<T> *vertexlist<T>::getuncolouredlist(void)
{
    AList<T> *list = new AList<T>();
    vertex<T> *ptv = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
...
template<class T> bool vertexlist<T>::colourable(vertexlist<T> *nodes, int col)
{
    vertex<T> *ptv = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
    {
        if ((*ptv).getcolour() < 0)
        {
            ...
            if (edges == 0)
                return true;
            if ((*edges).is_colourable(nodes, col))
                return true;
        }
        ptv = (*ptv).getnext();
    }
...
template<class T> void vertexlist<T>::prettyprint(void)
{
    ...
    for (int i = 0; i < numvertices; i++)
    {
        cout << "Edges with origin " << (*ptv).getitem() << " with colour " << (*ptv).getcolour() << " :\n";
...
template<class T> void vertexlist<T>::uncolour(void)
{
    vertex<T> *ptv = (*dummy).getnext();
    for (int i = 0; i < numvertices; i++)
    {
...
template<class T> int neighbour<T>::getcolour(vertexlist<T> *vertices)
{
    return (*vertices).getcolour(secondvertex);
}
...
template<class T> bool elist<T>::is_colourable(vertexlist<T> *vertices, int col)
{
    neighbour<T> *nb = getfirst();
    int colnb;
    for (int i = 0; i < length; i++)
    {
        colnb = (*nb).getcolour(vertices);
        if (colnb == col)
            return false;
        nb = (*nb).getnext();
    }
...
template<class T> void graph<T>::uncolour(void)
{
    (*vertices).uncolour();
    return;
}
...
template<class T> deepsearchtree<T> * graphtraversal<T>::colouring(void)
{
    int colour = 0;
    vertexlist<T> * vertices = graph<T>::vertices;
    deepsearchtree<T> *output = new deepsearchtree<T>;
    while ((*vertices).getlength() != (*vertices).numcoloured())
    {
        output = colourwith(colour);
        ++colour;
    }
    return output;
}

template<class T> void graphtraversal<T>::greedycolouring(void)
{
    int colour = 0;
    vertexlist<T> * vertices = graph<T>::vertices;
    ...
    while ((*vertices).getlength() != (*vertices).numcoloured())
    {
        samecolour = (*vertices).colourable(vertices, colour);
        if (samecolour)
            greedy(colour);
        else
            ++colour;
...
template<class T> deepsearchtree<T> * graphtraversal<T>::colourwith(int colour)
{
    if (colour < 0)
    {
        cout << "Colour must be non-negative.\n";
        return 0;
    }
    vertexlist<T> * vertices = graph<T>::vertices;
    deepsearchtree<T> *output = new deepsearchtree<T>;
    AList<T> *startlist = (*vertices).getlist();
    while ((*startlist).getlength() > 0)
    {
        int numvertices = (*startlist).getlength();
        srand(time(0));
        int index = rand() % numvertices;
        T startnode = (*startlist)[index+1];
        (*startlist).discard(startnode);
        dfsnew(startnode, startlist, output, colour);
    }
    return output;
}

template<class T> void graphtraversal<T>::greedy(int colour)
{
    if (colour < 0)
    {
        cout << "Colour must be non-negative.\n";
    ...
    vertexlist<T> * vertices = graph<T>::vertices;
    AList<T> *list = (*vertices).getuncolouredlist();
    int numvertices = (*list).getlength();
    srand(time(0));
    int index = rand() % numvertices;
    T node = (*list)[index+1];
    elist<T> * neighbours = (*vertices).edges(node);
    if (neighbours != 0)
    {
        if ((*neighbours).is_colourable(vertices, colour))
            (*vertices).setcolour(node, colour);
    }
    else
    {
        (*vertices).setcolour(node, colour);
...
template<class T> void graphtraversal<T>::dfsnew (T startnode, AList<T> *startlist, deepsearchtree<T> *output, int colour)
{
    vertexlist<T> * vertices = graph<T>::vertices;
    ...
    _dfs(startnode, startnode, startlist, vertices, output, markedvertices, dfspos, finishingtime, colour);
    return;
}

/* auxiliary functions for depth-first search */
template<class T> void graphtraversal<T>::_dfs(T node1, T node2, AList<T> *startlist, vertexlist<T> * vertices, deepsearchtree<T> * output, markerlist<T> * markedvertices, int &dfspos, int &finishingtime, int colour)
{
    elist<T> * neighbours = (*vertices).edges(node2);
    if (neighbours != 0)
    {
        if ((*vertices).getcolour(node2) == -1)
        {
            if ((*neighbours).is_colourable(vertices, colour))
                (*vertices).setcolour(node2, colour);
        }
        neighbour<T> * cursor = (*neighbours).getfirst();
        for (int i = 0; i < (*neighbours).getlength(); i++)
        {
            ...
            {
                (*startlist).discard(nextnode);
                _traverse_treeedge(node2, nextnode, output, dfspos);
                (*markedvertices).mark(nextnode);
                _dfs(node2, nextnode, startlist, vertices, output, markedvertices, dfspos, finishingtime, colour);
            }
            cursor = (*cursor).getnext();
        }
    }
