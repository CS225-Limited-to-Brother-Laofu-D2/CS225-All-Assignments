...
template<class T> int hashset<T>::getsize(void)
{
    return maxsize;
}

template<class T> void hashset<T>::setunion(hashset<T> *set)
{
    int num = (*set).getsize();
    T item;
    for (int i = 1; i <= num; i++)
...
template<class T> void hashset<T>::intersection(hashset<T> * set)
{
    int num = maxsize;
    T item;
    for (int i = 0; i < num; i++)
    {
        item = reprarray[i];
        if (item != 0 && item != pt_nil && not ((*set).member(item)))
            remove(item);
    }
    return;
}

template<class T> void hashset<T>::difference(hashset<T> * set)
{
    int num = (*set).getsize();
    T item;
    for (int i = 1; i <= num; i++)
