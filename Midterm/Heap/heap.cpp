...
template<class T> void MaxHeap<T>::build_heap_rec(AList<T> &array, int ind)
{
    int n = array.getlength();
    for (int i = 1; i <= n; i++)
    {
    ...
    this->build_heap2(ind);
    return;
}

template<class T> void MaxHeap<T>::build_heap2(int ind)
{
    if (4 * ind <= n)
    {
        this->build_heap_rec(2 * ind);
        this->build_heap_rec(2 * ind + 1);
    }
    this->sift_down(ind, n);
    return;
}
...
int main(){
    ...
    AList<int> input_array1;
    for (int i=0; i<10; i++){
        input_array1.append(input_list[i]);
    }
    ...
    MaxHeap<int> max_heap1, max_heap2;
    max_heap1.build_heap(input_array1);
    max_heap2.build_heap(input_array2);
    cout<< max_heap.max() << "\n\n";
    max_heap1.print_elements();
    cout<< max_heap.max() << "\n\n";
    max_heap2.print_elements();
