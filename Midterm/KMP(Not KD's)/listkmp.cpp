...
template<class T> void KMPlist<T>::create_next(void)
{
    ...
    int i = 2 , j = 1; // Here i represent the origin string, while we copy a string using "j" as the index. (Starting from 1)
    while(i <= this->getlength())
    {
        if (j == 1 || this->getitem(i - 1) == this->getitem(j - 1)) // Consider a string is comparing with itself.
        {
            // We can't do this->nextlist[i] = j, there's a optimization, for example, "abcabc" the next for second "a" should be 0 because "b == b".
            this->nextlist[i] = j;
            if(this->getitem(i) == this->getitem(this->nextlist[i]))
            {
                this->nextlist[i] = this->nextlist[j]; // In this case, the next of the origin string should be the same as the next value of the copied string.
            }
            i++;
            j++;
        }
        ...
        {
            j = 1; // The copied string mismatch, then compare from the beginning.
            this->nextlist[i] = j;
            if(this->getitem(i) == this->getitem(this->nextlist[i]))
            {
                this->nextlist[i] = this->nextlist[j]; // In this case, the next of the origin string should be the same as the next value of the copied string.
