...
template<class T> void AList<T>::insert( T value)
{
    if (numitems == maxsize)
        allocate();
    int i = 0;
    ...
    for (i = 0; i < num; i++ ){
        if ( i == 0){
            if (value == NULL){
             ...
             if (value->finishtime() > reprarray[0]->finishtime()){
                index = 0;
                 break;
             }
        }
        if (value->finishtime() <= reprarray[i]->finishtime()){
            if (i+1 == num){
                index = i+1;
                break;
            }
            if (value->finishtime() > reprarray[i+1]->finishtime()){
                index = i+1;
                break;
            }
        }
...
template<class T> AList<deeptreenode<int>*>* deepsearchtree<T>::tsort(void)
{   AList<deeptreenode<T>*>* result = new AList<deeptreenode<T>*>();
    if (first == 0)
    {
        return result;
    }
...
{
    vertexlist<T> * vertices = digraph<T>::vertices;
    if ((*vertices).contains(startnode) == false)// 先检查图中startnode是否存在
    {
        cout << "There is no vertex " << startnode << " in the graph.\n";
        return 0;
    }
    deepsearchtree<T> * output = new deepsearchtree<T>;   // 创建DFS生成树
    markerlist<T> * markedvertices = new markerlist<T>;   // 创建标记顶点集合
    int dfspos = 1;                                       // 初始化计数器dfpos 和 finishingtime
    int finishingtime = 1;
    (*markedvertices).mark(startnode);                    // 将startnode放入标记集合(相当于被标记)
...
{   // node1 是上一层节点, node2是当前层

    elist<T> * neighbours = (*vertices).edges(node2); // 当前顶点(node2)的所有邻点(链表),进入了下一层
    if (neighbours != 0)
    {
        neighbour<T> * cursor = (*neighbours).getfirst();
        for (int i = 0; i < (*neighbours).getlength(); i++) //当邻点非空时,遍历邻点
        {
            T nextnode = (*cursor).getitem();
            if ((*markedvertices).ismarked(nextnode) == true) // 如果当前邻点已被标记,略过该邻点
                _traverse_nontreeedge(node2, nextnode);
            else                                             // 如果当前邻点未被标记, 把node2放入生成树新的节点(若node2不在树中),并标记当前邻点
            {                                                // 并从当前邻点开始dfs
    ...
    // 所有邻点都被遍历了,node2的dfs已完成, set finishing time
...
{   
    // node2 的dfs已经完成, 所有邻点都被dfs过了,回到上一层(node1)
    if ((*output).occurs(node2) == false)
    {
        (*output).add(node2, dfspos, finishingtime);
    }
    else
        (*output).setfinishtime(node2, finishingtime); //若node2已经在树里了,更新finishingtime
    finishingtime++;
    return;
}

template<class T> AList<deeptreenode<T>*>* digraphtraversal<T>::tsort(T startnode){
    deepsearchtree<int> * yourdfs = dfs(startnode);
    yourdfs->prettyprint();
    AList<deeptreenode<int>*>* result = yourdfs->tsort();
    return result;
}